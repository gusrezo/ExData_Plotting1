...
## We're going to set the locale to english, so any reviewer can check that the 
## labels for the days are properly displayed in english.
Sys.setlocale("LC_ALL","English")

## Since the date and time columns are factors, 
## we're going to add a new variable with Date and Time stored as a POSIXlt
epc12feb$datetime <- strptime(paste(as.character(epc12feb$Date), 
                                    as.character(epc12feb$Time)),  "%d/%m/%Y %H:%M:%S" ) 
...
with(epc12feb, plot(datetime, Global_active_power, type = "l", 
                    xlab="",ylab = "Global Active Power (kilowatts)"))

dev.off()

## Check that in your working directory the file plot2.png has been created. End plot2